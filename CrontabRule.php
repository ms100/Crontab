...
class CrontabRule{
    ...
    static public function check_time_rule($time_rule){
        ...
        return self::parse_time_rule_part($parts[0], 0, 59)//分
        && self::parse_time_rule_part($parts[1], 0, 59)//时
        && self::parse_time_rule_part($parts[2], 1, 31)//日
        && self::parse_time_rule_part($parts[3], 1, 12)//月
        && self::parse_time_rule_part($parts[4], 0, 6);//周（0周日）
    }
    ...
    static protected function parse_time_rule_part($part, $f_min, $f_max){
        ...
        if(false !== strpos($part, ',')){
            $arr = explode(',', $part);
            foreach($arr as $v){
                $tmp = self::parse_time_rule_part($v, $f_min, $f_max);
                if(!$tmp) return false;
    ...
    static protected function get_mday_by_wday($wday, $mon, $year){
        ...
        if(is_array($wday)){
            if(count($wday) <= 4){
                foreach($wday as $w){
                    $res = array_merge($res, self::get_mday_by_wday($w, $mon, $year));
                }
            }else{
                ...
                foreach($wday as $w){
                    $res = array_diff($res, self::get_mday_by_wday($w, $mon, $year));
                }
            }
    ...
    static public function get_next_time($time_rule, $timestamp = 0){
        ...
        $parts = explode(' ', $time_rule);
        $cron_range = array();
        $cron_range['minutes'] = self::parse_time_rule_part($parts[0], 0, 59);//分
        $cron_range['hours'] = self::parse_time_rule_part($parts[1], 0, 59);//时
        $cron_range['mday'] = self::parse_time_rule_part($parts[2], 1, 31);//日
        $cron_range['mon'] = self::parse_time_rule_part($parts[3], 1, 12);//月
        $cron_range['wday'] = self::parse_time_rule_part($parts[4], 0, 6);//周（0周日）
        ...
        if(in_array($date['minutes'], $cron_range['minutes']) && in_array($date['hours'], $cron_range['hours']) && in_array($date['mon'], $cron_range['mon']) && ((($parts[2] == '*' || $parts[4] == '*') && in_array($date['mday'], $cron_range['mday']) && in_array($date['wday'], $cron_range['wday'])) || (in_array($date['mday'], $cron_range['mday']) || in_array($date['wday'], $cron_range['wday'])))){
            ...
            if($index + 1 < count($cron_range['minutes'])){
                ...
                if($index + 1 < count($cron_range['hours'])){
                    ...
                    //$append_mday = self::get_mday_by_wday($cron_range['wday'], $date['mon'], $date['year']);
                    //$mday_range = array_intersect($cron_range['mday'], $append_mday);
                    if($parts[4] == '*'){
                        $mday_range = $cron_range['mday'];
                    }elseif($parts[2] == '*'){
                        $mday_range = self::get_mday_by_wday($cron_range['wday'], $date['mon'], $date['year']);
                    }else{
                        $append_mday = self::get_mday_by_wday($cron_range['wday'], $date['mon'], $date['year']);
                    ...
                    if($index + 1 < count($mday_range)){
                        ...
                        while(1){
                            ...
                            //$append_mday = self::get_mday_by_wday($cron_range['wday'], $next_date['mon'], $next_date['year']);
                            //$mday_range = array_intersect($cron_range['mday'], $append_mday);
                            if($parts[4] == '*'){
                                $mday_range = $cron_range['mday'];
                            }elseif($parts[2] == '*'){
